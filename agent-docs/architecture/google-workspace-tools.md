...
Support Gmail and Google Calendar actions in chat without exposing raw Google APIs directly to the model, and require manual approval before any mutation is executed.
...
All Gmail and Calendar writes use a draft-first workflow:
...
The assistant must never claim a mutation succeeded after creating a draft. Draft creation is not execution.
